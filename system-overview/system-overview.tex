...
The requirements in section \vref{section:requirements} stated that the \Gls{barricelli} should use an \gls{FPGA} and an microcontroller as system components.
...
The \gls{SCU}, or System Control Unit, is an Energy Micro EFM32GG990F1024 microcontroller which administrates communication between the gls{FPGA} and the outside world.
...
Having a separate microcontroller to perform these tasks minimises the complexity of the design implemented on the \gls{FPGA}, which means that more resources can be used to create a performant and clean custom processor design.
