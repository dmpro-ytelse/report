...
\subsubsection{Data Memory Controller} 
...
This functionality is divided into two different kind of controllers; one located in each fitness core, and one main controller responsible for the actual data memory

\subsubsection{Genetic Pool Controller} 
As with the memory, the fitness cores need to request the memory bus from a central arbitration unit. However, this unit is different compared to the \emph{data memory controller}. This controller is responsible for controlling the access to the rated and unrated pools. 
