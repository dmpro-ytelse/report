...
\todo{rewrite the following paragraph because it is about as concise as a sponge}
The assembler does not perform any optimizations such as instruction re-ordering.
This is because the forwarding unit in the CPU architecture resolves many of the same issues that the assembler would work around using instruction re-ordering. \todo{does the forwarding unit work around other issues (1), or only those solved by instruction re-ordering (2)? if 2 then don't use "such as" in this paragraph's first sentence. if 1 then list the other issues it resolves}
